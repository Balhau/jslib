...
BLauLib.UI.DatePicker.prototype._createTableDate = function(id) {
	...
	this.cells = $$('td', 56); // Numero de células 49, inclui dias do mes,
...
/**
 * Método privado ao objecto que pretende actualizar as células dos dias em função
...
/**
 * Este elemento UI constrói um conjunto de colunas com elementos do tipo floating div. A principal utilidade deste 
 * componente prende-se com o desenvolvimento de paineis compostos por pequenas mensagens.
 * 
 * @param {Number} Número de colunas a criar
 * @param {array} Array de objectos com os dados na sua representação JSON. O array tem a seguinte estrutura
 ...
 * representam o código em HTML do conteúdo a passar para as trẽs secções.
 * @param {String} String que contem o valor do id para o contentor e respectivos sub elementos de HTML
 * @param {Object} Objecto na notação json representado da seguinte forma
 * {"width":"val","colspace":"val","vspace":"val"}
 * onde width representa a largura do contentor, colspace o espaço entre colunas e vspace o espaço vertical entre entradas
 */
...
/**
 * Método privado que é utilizado para efectuar a construção dos objectos
 */
BLauLib.UI.Columns.prototype.Build=function(){
	...
	this.columns=[];
	//Construção dos div das colunas
	...
	//Adição das entradas às respectivas colunas
...
/**
 * Devolve o elemento DOM com as várias colunas de dados
 */
...
/**
 * @classDescription Este objecto representa uma classe vector que contem um conjunto de métodos
 * e propriedades para implementação de calculo vectorial
 */
...
Function.prototype.getArgs=function(){
   ...
   var fi=ts.indexOf('('); 
   var li=ts.indexOf(')');
   return ts.substring(fi+1,li).split(",").map(function(el){return el.trim();});
}
